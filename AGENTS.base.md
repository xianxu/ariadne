...
#### This Repo
- Simple work (inside the quick-flow shell — `sdlc change-code --help` prints the limits) → one file in `workshop/issues/` and no durable plan; complex → add a durable plan at `workshop/plans/NNNNNN-slug-plan.md`, authored via the **`superpowers-writing-plans`** skill (the canonical plan path — see §2). It's version-controlled and archived with the issue; never the harness builtin's ephemeral `~/.claude/plans/` file.
...
- **Entering planning:** run `sdlc start-plan` (after `claim`, before you design). It delivers the `at-plan` architectural principles (`ARCH-*`, see Core Design Principles) so the design accounts for them from the start — the forward counterpart to `change-code`'s plan-quality review — and points at the durable-plan skill + location for work outside the quick-flow shell. Re-run per design (agents don't reread). Flow: `claim → start-plan → (design; a durable plan via `superpowers-writing-plans` → `workshop/plans/` only outside the shell) → change-code → implement → close`. The gates infer the flow from what the design produced (#231): Mx milestones or a durable plan mean the full flow; neither means the quick flow — no plan-quality, no estimate, one small-diff review at close, which upgrades to the full review if the diff leaves the shell. The operator can pin it (`change-code --flow`). Your sequence of verbs is the same either way.
