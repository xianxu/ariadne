...
- **Checkpoint the design as it lands (#206):** `sdlc issue sync --issue N` commits the issue body (`## Spec`/`## Plan`/`## Log`) under a message naming the issue. It does **not** push — a local commit, no network, cheap enough to run whenever the design moves. Nothing else commits the body until `change-code`, so a compaction or a closed terminal between `claim` and `change-code` loses the design outright; `claim` publishes only the *reservation* (an ID and a name), which is deliberate and stays. Run it after a brainstorm lands, after a decision you'd hate to re-derive, and before any long-running tool call. `change-code` runs the same sync **with** the push at the end of planning — but only over what survived to it.
