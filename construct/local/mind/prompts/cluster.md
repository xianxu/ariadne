...
You are aggregating candidate patterns extracted from many segments of a user's Claude Code transcripts into clusters that become rules in an activity-typed `mind-<activity>` skill.
...
2. **Activity scoping.** Only cluster patterns that share the same `activity`. Do not merge across activities. The rule is going into a `mind-<activity>` skill that's loaded only when that activity is detected, so cross-activity merging is wrong.
