...
- **SpanThroughput** — `SpanThroughput(rows []estimate.LedgerRow, from, to string) (SpanMeasure, error)`: pure math over parsed calibration-ledger rows → `{HoursPerWeek, Rows, UntrustedRows, Days}`. Inclusive day count; divide by `days/7.0` (partial weeks well-defined). Includes `window_trusted=no` rows (their measured hours are real; excluding them under-counts worse) and reports their count so the bless output can warn. **Reuses the EXISTING `estimate.LedgerRow` + `estimate.ParseRows` (`internal/estimate/ledger.go`)** — the ledger already has one parser; adding another would redeclare the type and violate ARCH-DRY. `LedgerRow.Actual` and `.Date` are strings in the existing shape; `SpanThroughput` parses them (bad float/date in a row → skip with a counted note, matching the parser's tolerant stance).
...
- [ ] **Step 1: Write the failing tests** — table tests: `SpanThroughput` over `estimate.ParseRows` fixture text (the REAL 10-column ledger shape — reuse a snippet of real rows): 28-day span → `Σactual/4.0`; 27-day span divides by `27/7.0`; rows outside span excluded; untrusted counted but included in the sum; a row with an unparsable actual → skipped + counted; empty span → error. `ParseBaselineTSV` (last row current, comments skipped, bad float → error) + `RenderBaselineRow` round-trip.
