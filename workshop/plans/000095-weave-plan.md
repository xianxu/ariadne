...
- **Add a `weave link <path>` verb** that records `substrate <path>` **verbatim** (replaces the `tool` action's hardcoded `substrate ../ariadne`), so a test setup captures the real path it was given.
...
- **Adopt the side-by-side migration as the integration test** (added to Done-when): clone a real derivative (e.g. parley) + the ariadne worktree anywhere, `weave link <ariadne-path>`, migrate it off `setup.sh`, both on branches — production repos untouched. Stronger than golden-diff alone.

Test/rollout progression (validated as enabled by the above): (1) self-walk in the ariadne worktree, golden-diff vs setup.sh + a live session; (2) spawn a derivative via `weave link`; (3) the side-by-side migration above.
