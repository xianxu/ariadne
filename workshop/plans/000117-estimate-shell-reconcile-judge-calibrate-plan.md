...
- **2026-06-17 (M2 review, FIX-THEN-SHIP):** Folded the boundary-review findings: (a) the vocab↔helptext drift guard is now **bidirectional** (forward: every `vocab.go` slug is documented; reverse: no stray slug in the doc) — Task 8's "exactly the slug set" now holds; (b) added a unit test for `runEstimateQualityJudge`'s skip-on-no-block branch; (c) ARCH-DRY: extracted `issue.SectionBody(body, heading)` and routed the spec / done-when / estimate / sizing section extractions through it (`PlanSection` keeps its own regex — it needs byte offsets), removing the 4th duplicate regex. Noted: the estimate-quality prompt references `estimate-logic-v2` by name + full issue content (Task 7 said "reads the model doc") — deliberate base-layer posture, since the brain model doc is absent/encrypted downstream. Deferred (Minor, non-blocking): `estimateReconRefusal`'s `estimate_hours` parse-error→0 edge under `--no-estimate`; a `## Estimate` stub in the `sdlc issue new` scaffold; `helptext/estimate.md` reachable only by file path.
