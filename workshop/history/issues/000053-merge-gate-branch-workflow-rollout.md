...
**Phase C — finish generic mechanism (#1 / #52 M2):** ⏸ DEFERRED (operator, 2026-05-31)
- ⏸ **DEFERRED — owned by #52 M2, not this conductor.** `make remote-init` (gh repo create + opt-in required-check). Advisory + merge-on-green is a fine resting point. Turning on a required-check removes the `sdlc push` direct-on-main escape (protected branch) and still isn't full teeth — CI runs gate code from the PR's own tree, so tamper-proof enforcement also needs CI to run the gate from a trusted/pinned ref (Codex #52 finding #1). Revisit when you actually want the gate to block rather than warn. (Not a #53 checkbox — #53's sequencing job is done; this lives in #52.)
