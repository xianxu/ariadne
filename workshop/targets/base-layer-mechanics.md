...
A skill's BODY may itself be regenerated at compile time — the *dynamic-skill* maintenance stage (ariadne#111, reshaped by ariadne#115): a package carrying an executable `.dynamic-skill` is exec'd by `weave compile` (before this compose stage) to **materialize its `SKILL.md` per-repo at `construct/generated/<dir>/SKILL.md` — gitignored everywhere + regenerated every compile**, never committed. This is **maintenance** (keeping the body current), upstream of and distinct from **composition** (the union math here) and **lowering** (the per-harness symlinks) — it changes WHAT a skill's body says, never HOW it composes. weave's IO is therefore filesystem + this one bounded `.dynamic-skill` exec seam (NOT filesystem-only); the exec runs the **all-layers visible-set** (NOT leaf-only), adapted-excluded, injected + fake-tested, with cwd = the COMPILING repo's root + repo-relative `--output construct/generated/<dir>` so output is **leaf-rooted** (an ancestor's tree is never mutated by a derivative's compile). The skill entry is emitted from the tracked marker (discovered even in a fresh clone). The owning subsystem invariant lives in [[skill-system]]; details in [weave atlas](../../atlas/workflow/weave.md).

Datatype-prototype propagation is **NOT a file-op or symlink** (#115 retired the `symlink construct/datatype` manifest row): each layer owns its own `construct/datatype/`, and the `datatype` binary reads the **DAG-merged union** across the layer graph. The DAG walk itself is the shared module-level library `pkg/layergraph` (the transitive `construct/deps` topology) — imported by BOTH weave and the `datatype` binary, so the two DAG-aware tools never diverge on "repo R's layer graph." This is the pensive's "single mechanism" now realized cross-tool: `datatype` is the first SECOND-consumer of the shared layer-graph walk. (A sibling `pkg/frontmatter` shares the flat-YAML `description:` parser between weave + datatype.)
