...
- [ ] `make remote-init` (gh repo create + opt-in required-check). **Deliberately punted to a future task:** advisory + merge-on-green is a fine resting point. Turning on a required-check removes the `sdlc push` direct-on-main escape (protected branch) and still isn't full teeth — CI runs gate code from the PR's own tree, so tamper-proof enforcement also needs CI to run the gate from a trusted/pinned ref (Codex #52 finding #1). Revisit when you actually want the gate to block rather than warn.
