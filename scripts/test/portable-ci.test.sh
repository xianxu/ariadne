...
with tempfile.TemporaryDirectory(prefix='portable ci ') as tmp:
    ...
    executable(commands/'go','echo candidate-build >> "$FIXTURE/leaf/events"\n[ "$1" = build ] && [ "$2" = -o ]\ncp "$FIXTURE/candidate" "$3"\n')
    executable(commands/'brew','echo "brew:$*" >> events\nif [ "$1" = tap ] && [ "${TAP_MISSING:-0}" = 1 ]; then exit 1; fi\nif [ "$1" = install ]; then mkdir -p "$FIXTURE/formula/bin"; cp "$FIXTURE/candidate" "$FIXTURE/formula/bin/weave"; fi\nif [ "$1" = --prefix ]; then echo "$FIXTURE/formula"; fi\n')
    # Only `clone` is faked: the merge-check blocks still need real git.
    executable(commands/'git','if [ "$1" = clone ]; then echo clone >> events; dest="${@: -1}"; mkdir -p "$dest"; printf \'brew "go"\\n\' > "$dest/Brewfile"; exit 0; fi\nPATH=/usr/bin:/bin exec git "$@"\n')
    ...
    rows=run(); assert rows==['brew:tap xianxu/ariadne','brew:install xianxu/ariadne/weave','brew:--prefix xianxu/ariadne/weave','compile','setup','checked'],rows
    # Tap unpublished (#250, until #241): build the candidate from ariadne source.
    env['TAP_MISSING']='1'; (leaf/'events').write_text(''); (leaf/'scripts/run-merge-checks.sh').unlink()
    rows=run(); assert rows[:2]==['brew:tap xianxu/ariadne','clone'] and rows[2].startswith('brew:bundle --file=') and rows[2].endswith('/ariadne-source/Brewfile') and rows[3:]==['candidate-build','compile','setup','checked'],rows
    del env['TAP_MISSING']
...
assert '::warning::' in workflow and '#241' in workflow
...
print('PASS CI actual blocks: published/unpublished-tap/source gateway, compile before hooks/checks, failures')
PY
