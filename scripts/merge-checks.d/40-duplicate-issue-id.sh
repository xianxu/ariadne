...
# Build sdlc from THIS checkout rather than trusting a PATH binary: CI has none
...
if ! go build -o "$tmp/sdlc" ./cmd/sdlc 2>/dev/null; then
    # A derivative repo has no ./cmd/sdlc of its own; it consumes the base
    # layer's binary. Nothing to build here, nothing to check.
