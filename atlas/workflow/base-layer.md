...
- **Construct system**: `construct/scripts/`, `construct/datatype/` (symlinked) — skill tooling + datatype prototypes. (`construct/local/` + `construct/adapted/` are ariadne's OWN skill dirs, read by derivatives through the weave layer walk — NOT installed by symlink since #104 M3.)
