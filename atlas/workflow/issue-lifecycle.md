...
6. **Flows (#231)**: `change-code` records `flow: {kind, provenance}` on the issue. Mx milestones, or a design (`## Spec` + `## Plan` + the durable plan, in lines) past the shell's design limit → **full** (every gate below). Neither → **quick**: none of change-code's gates, no estimate, a plan optional, and one review at close with the small-diff recipe. The hard shell (`sdlc change-code --help`) is checked at change-code on what exists then and again at close on the real diff and the design as it stands: a quick issue outside it is upgraded to full. The operator pins with `--flow`. The agent's verb sequence is the same on both — the gates branch, not the agent.
