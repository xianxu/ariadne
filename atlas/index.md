...
- [Gate State](workflow/gate-state.md) — how an SDLC gate remembers what it asked for: the `finding` CUE noun, the schema'd ` ```findings ` handoff, binary-assigned stable ids, the pure block/pass decision, the content-hash pass-through, and the plan-gate → boundary-review carry-forward (#187; #183 is the second intended consumer)
