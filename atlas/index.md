...
- [sdlc Binary](workflow/sdlc-binary.md) — unified checkpoint-guard binary (`cmd/sdlc/`) guarding the workflow from claim → ship (incl. the `issue` group with its body-commit verb `issue sync` (#206), `active-time` the in-binary v3 attribution engine #110, and the read-only `resolve`/`open` ref resolver #144, `migrate` — cross-repo artifact move with ref rewrite #179) replacing the Make-target surface; embedded `--help` per subcommand; fresh-context judges for anti-collusion
