...
- [sdlc Binary](workflow/sdlc-binary.md) — unified checkpoint-guard binary (`cmd/sdlc/`) guarding the workflow from claim → ship (incl. the `issue` group with its body-commit verb `issue sync` (#206), `active-time` the in-binary v3 attribution engine #110, the read-only `resolve`/`open` ref resolver #144, and `migrate` — cross-repo artifact move with ref rewrite #179) replacing the Make-target surface; embedded `--help` per subcommand; fresh-context judges for anti-collusion
- [Workspace Identity](workflow/workspace-identity.md) — shared Git repository/workspace resolver, numbered slots, JSON contract and SDLC path ownership (#242).
- [Branching, Moving and Refreshing Slots](workflow/workspace-branching.md) — Git procedures for captured-source issue branches, moving a branch between slots, and explicit resting-branch fast-forwards (#245, #248).
...
- [Architecture Principles](workflow/architecture-principles.md) — single-source `ARCH-*` registry pushed through planning/review gates and `sdlc arch-principles`; covers how an entry is added, and the deferred file for principles documented but deliberately not gated.
...
- [pkg/weaveownership](../pkg/weaveownership/) — shared read-only generated-output inventory and identity proof, used by weave retirement and safe propagation index migration (#239).
