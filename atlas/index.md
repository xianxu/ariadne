...
- [Vocabulary Layer](workflow/vocabulary.md) — formal CUE models of nouns + lifecycles (`construct/vocabulary/`); the single source consumers derive from. `issue.cue` is the first (#122, M1–M2 landed: model + `cmd/vocabulary` + weave wiring + committed embed json; sdlc consumption is M3). Propagates like datatype.
