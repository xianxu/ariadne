...
- [sdlc Binary](workflow/sdlc-binary.md) — unified checkpoint-guard binary (`cmd/sdlc/`), 13 verbs (incl. the `issue` group + `active-time`, the in-binary v3 attribution engine #110, + the `bench` group #119) replacing the Make-target surface; embedded `--help` per subcommand; fresh-context judges for anti-collusion
- [Benchmark harness](workflow/bench.md) — `sdlc bench`: run different coding agents (claude/codex/…) on the SAME frozen task in isolated worktrees, grade by measured objective signals + blind head-to-head judging (#119). Base-layer harness, repo-local task library.
...
- [pkg/layergraph + pkg/frontmatter](../pkg/) — module-level shared libraries (#115 M1): the transitive `construct/deps` layer-graph walk (`pkg/layergraph`, imported by BOTH weave and the `datatype` binary — the "single mechanism" so two DAG-aware tools never diverge on topology) + a flat-YAML `description:` parser (`pkg/frontmatter`). The DAG-merged datatype prototypes + per-repo gitignored dynamic-skill materialization (#115) are documented in [Data Artifacts](workflow/data-artifacts.md) + [weave atlas → Dynamic skills](workflow/weave.md).
