...
- **Two trackers:** `workshop/issues/` is the internal tracker (Spec/Plan/Log per issue); GitHub Issues are an external inbox (bug reports + requests from non-contributors). Triage GH → `workshop/issues/<N>-slug.md` (`deps: <repo>#N`) when work starts. Don't `gh issue create`, use workshop/issues directly.
