...
func TestNormalizeVantage_EquivalentFakeVantages(t *testing.T) {
	...
	reader := &vantageStub{
		...
		worktrees: topologyPorcelain(primary, linked),
		common:    common,
	}
...
func TestNormalizeVantage_SkipsUnresolvableUnrelatedListedWorktree(t *testing.T) {
	...
	porcelain := topologyPorcelain(primary) + "worktree " + missing + "\x00HEAD deadbeef\x00branch refs/heads/locked\x00locked unavailable\x00\x00" + topologyPorcelain(healthy)
...
func TestNormalizeVantage_RefusesMissingContainingWorktree(t *testing.T) {
	...
	_, err := NormalizeVantage(&vantageStub{
		...
		worktrees: topologyPorcelain(other),
	...
	for _, suffix := range []string{"\n", "\r"} {
		t.Run(fmt.Sprintf("%q", suffix), func(t *testing.T) {
			...
			got, err := NormalizeVantage(&vantageStub{
				...
				worktrees: topologyPorcelain(primary),
				common:    common,
			}, primary)
			if err != nil {
				t.Fatal(err)
...
func (execGitReader) GitInDir(dir string, args ...string) ([]byte, error) {
	...
	cmd.Dir = dir
	cmd.Env = append(os.Environ(), "GIT_CONFIG_NOSYSTEM=1", "GIT_CONFIG_GLOBAL=/dev/null", "GIT_CONFIG_COUNT=1", "GIT_CONFIG_KEY_0=core.hooksPath", "GIT_CONFIG_VALUE_0=/dev/null")
...
func topologyPorcelain(primary string, linked ...string) string {
	var records []string
