...
- **principle:** A thing that is created has a lifecycle, and its end is part of
  ...
  residue**: that entry budgets a component while it is working, and every
  constraint in it is a rate or a level someone can feel. This one governs what
  is left behind once the work is done, where the cost accrues while nothing is
  happening and every individual write sat comfortably inside its envelope. A
  perfectly budgeted component with no removal path passes ARCH-CONSTRAINTS and
  reaches a cliff anyway.
