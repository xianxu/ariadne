...
func runJudge(stdout, stderr io.Writer, categoryArg string, f *judgeFlags) error {
	...
	diff, changed, err := collectDiff(cat, base, head)
	if err != nil {
...
//
// Excludes workshop/issues/ + workshop/history/ from the main diff for
// dry/pure/specs/milestone-review (so prose churn doesn't dominate the
// review). The Plan category gets the inverse — only issue files.
func collectDiff(cat judge.Category, base, head string) (diff string, changedIssues []string, err error) {
	args := []string{"diff", base}
	if head != "" {
		args = append(args, head)
	}
	// Path exclusions match scripts/pre-merge-checks.sh:
	//   git_diff_context  → excludes issues/ and history/
	//   git_diff_context_issues → only issues/
	switch cat {
	case judge.Plan:
		args = append(args, "--", "workshop/issues/*.md")
	default:
		args = append(args, "--", ":!workshop/issues/", ":!workshop/history/")
	}
	...
	if cat == judge.Plan {
		...
		nameArgs = append(nameArgs, "--", "workshop/issues/*.md")
